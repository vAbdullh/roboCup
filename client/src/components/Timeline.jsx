                ...
                {events.map((event, index) => (
                    <li key={index} className="relative mb-6 xl:mb-0 xl:w-60" data-aos="fade-right" data-aos-duration={`10${index}0`}>
                        <div className="flex items-center">
                            <div
                                className={`z-10 flex items-center justify-center w-6 h-6 rounded-full xl:ring-8 shrink-0
                                ${event.isCompleted ? 'bg-green-400 ring-green-600' : 'bg-blue-100 ring-white dark:bg-blue-900 dark:ring-gray-900'}`}
                            >
                                {event.isCompleted ? (
                        ...
                        <div className="mt-3 xl:pe-8">
                            <p className={`text-lg ${event.isCompleted ? 'text-gray-400 line-through' : 'text-white'}`}>
                                {event.title}
                            </p>
                            <time className="block mb-2 text-sm font-extralight leading-none text-gray-200">{event.date}</time>
                            <p className={`text-base font-extralight ${event.isCompleted ? 'text-gray-400 italic' : 'text-gray-300'}`}>
                                {event.description}
                            </p>
                        </div>
                    </li>

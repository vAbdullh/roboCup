    ...
    return (
        <div className="grid grid-cols-1 gap-6">
            <button disabled={loading} onClick={handleGetLogs}
                ...
                {loading && <Loader size={24} className="animate-spin" />}Refresh
            </button>
            ...
            <div className="grid grid-col-1 md:grid-cols-2 xl:grid-cols-3 gap-2 fade-in">
                {loading ? (<Loading />) : logs.length === 0 ? (
                    ...
                    .map((log, index) => (
                        <div key={index} className="flex flex-col gap-4 p-6 rounded-xl col-span-1 bg-white shadow-md font-extralight tracking-wide">
                            <p><span className="font-normal text-xl">Details:</span> {log.details}</p>
                            <p><span className="font-normal text-xl">User:</span> {log.user}</p>
                            <p><span className="font-normal text-xl">Time:</span> {new Date(log.timestamp).toLocaleString()}</p>
                        </div>
                    ))
                )}
            </div>
